...
export default function Letter({ usersData }) {
  const [content, setContent] = useState(usersData.letter_template);
  ...
  const handleSubmit = async (e) => {
    ...
    const res = await fetch(apiRoute, {
      ...
      body: JSON.stringify({
        id: usersData._id,
        updateFields: { 'letter_template': content },
      })
    });
    ...
    if (res.ok) {
      ...
      setTimeout(() => {
        setSeverity('success');
        handleMsg('Letter template has been saved');
        setTimeout(() => {
          setIsLoading(false);
          router.push(`/`)
        }, 1500);
